...
export default function Signin() {
  class ShellEventEngine extends EventTarget {
    // @dev event helpers
  ...
  useEffect(() => {
    ...
    const onChange = () => {
      ...
      // iframe.div.div.style.display = '300px'
  ...
  const onReCAPTCHAChange = async (captchaCode) => {
    ...
    try {
      console.log('recaptchaRef after triggered: ', recaptchaRef.current)
  ...
  return (
    <div>
      <main className={styles.main}>
        <h1 className={styles.title}>Lootex v3 Landing Page</h1>
        <div>isMobile: {isMobile ? 'true' : 'false'}</div>
        <button onClick={() => recaptchaRef.current.execute()} className='btn'>
          TEST recaptcha
        </button>
